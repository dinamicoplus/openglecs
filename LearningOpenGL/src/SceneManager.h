...
extern struct TexturedModelComponent;
...
struct Scene {
    ...
    std::array<EntityID, MAX_ENTITIES> m_Entities{ -1 }; // Current Entities in the scene
...
public: // Cambiar a public para facilitar el uso
    static EntityID NewEntity(Scene* scene) {
        if (scene->entityIdx >= MAX_ENTITIES) return static_cast<EntityID>(-1); // Verificación de límite
        
        EntityID newEntity = scene->entityIdx; // Simplificar: usar directamente el índice como EntityID
        scene->m_Entities[newEntity] = newEntity;
        ComponentMask emptyMask; // Crear máscara vacía
    ...
    static ComponentArray<T>* GetComponentArray(Scene* scene) {
        ...
        return nullptr; // Añadir caso por defecto
    }

    template<typename T>
    static T* AssignComponent(Scene* scene, EntityID entity, T component) {
        if (entity >= MAX_ENTITIES) return nullptr; // Verificación de límite
        
        // Obtener componente array
        ComponentArray<T>* array = GetComponentArray<T>(scene);
        if (!array) return nullptr;
        
        // Guardar máscara anterior para actualizar el mapa
        ...
        // Actualizar máscara y mapa de componentes
        size_t componentTypeID = ComponentType.at(std::type_index(typeid(T)));
        scene->m_ComponentMaskToIdxMap[oldMask].erase(entity); // Quitar entidad de la máscara anterior
        scene->m_EntityMasks[entity].set(componentTypeID); // Actualizar máscara
        scene->m_ComponentMaskToIdxMap[scene->m_EntityMasks[entity]].insert(entity); // Agregar a nueva máscara
    ...
    static T* AssignComponent(Scene* scene, EntityID entity) {
        return AssignComponent(scene, entity, T{}); // Reutilizar lógica existente
    }
    ...
    static void RemoveComponent(Scene* scene, EntityID entity) {
        if (entity >= MAX_ENTITIES) return; // Verificación de límite
        
        ComponentArray<T>* array = GetComponentArray<T>(scene);
        if (!array) return;
        
        // Guardar máscara anterior
        ComponentMask oldMask = scene->m_EntityMasks[entity];
        
        // Remover componente
        ComponentArrayManager::RemoveComponent<T>(array, entity);
        
        // Actualizar máscara y mapa
        size_t componentTypeID = ComponentType.at(std::type_index(typeid(T)));
    ...
    static void DeleteEntity(Scene* scene, EntityID entity) {
        ...
        if (entity != lastEntity) {
            ...
            // Actualizar mapa de máscaras para la entidad movida
    ...
    static std::vector<std::pair<EntityID, T*>> FilterComponentsByMask(Scene* scene, const ComponentMask& targetMask) {
        ...
        for (const auto& [mask, entities] : scene->m_ComponentMaskToIdxMap) {
            // Verificar si esta máscara incluye todos los bits de targetMask
...
public:
    ...
    static void ReserveComponentForEntity(ComponentArray<T>* componentArray, EntityID entity) {
        // Verificar si ya existe un índice para esta entidad
    ...
    static T* AddComponentToEntity(ComponentArray<T>* componentArray, EntityID entity, T component) {
        ...
        // Obtener índice actual para esta entidad
        auto it = componentArray->m_IndexToEntityMap.find(entity);
    ...
    static void RemoveComponent(ComponentArray<T>* componentArray, EntityID entity) {
        ...
        // Si no es el último elemento, mover el último a su posición
